...
tic.create = {
     ...
     * @return {Promise.<TICClient>} a Promise awaiting the initialized TICClient.
     */
    fromMasterClient: async (masterClient, password = masterClient.mam.channel.seed) => {
        const seeds = {}
        channelOrder.forEach(k => seeds[k] = mamClient.generateSeed())
        let { mam: mamState, channelRoot, iota } = masterClient

        if (password) {
            if (mamState.channel.mode != 'restricted' || mamState.channel.side_key != password) {
                ({ mam: mamState } = await mamClient.changeMode(mamState, 'restricted', password))
            }

            log.log('info', "Saving channel seeds in restricted channel")
            await asyncForStreams(async (key, seed) => {
                const result = await mamClient.publish(seed, mamState, iota, false) //seed is payload in restricted stream
                mamState = result.mamState
            }, seeds)
        }

        ({ mam: mamState, channelRoot } = await mamClient.changeMode(mamState, 'public'))
...
tic.init = {
    ...
    fromMasterSeed: async (masterSeed, password = masterSeed,  iota = mamClient.getIota()) => {
        ...
        if (password) {
            client = await mamClient.createMam(masterSeed, iota, 'restricted', password)
        } else {
           client = await mamClient.createMam(masterSeed, iota)
        }
     ...
     * @return {Promise.<TICClient>} a Promise awaiting the initialized TICClient.
     */
    fromMam: async (client, password = client.mam.channel.seed) => {
        let { mam: master, channelRoot, iota } = client
        const seeds = {}
        if (password) {
            let restrictedChannelRoot = undefined
            debug("Reading seeds from restricted channel")
            if (master.channel.mode != 'restricted' || master.channel.side_key != password) {
                ({ mam: master, channelRoot: restrictedChannelRoot } = await mamClient.changeMode(master, 'restricted', password))
            } else {
                restrictedChannelRoot = channelRoot
            }
            const { messages: seedMessages } = await mamClient.getChannelMessages(restrictedChannelRoot, master)
            ...
            ({ mam: master, channelRoot } = await mamClient.createMam(master.channel.seed, iota)) //await mamClient.changeMode(master, 'public'))
        }
     ...
     * @return {Promise.<TICClient>} a Promise awaiting the initialized TICClient.
     */
    fromSeeds: async (seeds, masterClient, password) => {
...
const readChannel = async (channelRoot, iota, put = "put", del = "delete") => {
...
const readChannelArray = async (channelRoot, iota, add = "add", remove = "remove") => {
    const { messages } = await mamClient.getMessages(channelRoot)
    const objects = messages.map(iota.utils.fromTrytes).map(JSON.parse)
...
tic.profile = {
     ...
     * @return {Promise.<string>} a Promise awaiting the root address of the attached message.
     */
     ...
     * @return {Promise.<Object>} a Promise awaiting the compiled content of the profile MAM channel.
     */
    get: async (channelRoot, iota) => readChannel(channelRoot, iota)
}
...
tic.contacts = {
     ...
     * @param {IotaClass} iota The iota client for the MAM client.
     * @return {Promise.<Array>} a Promise awaiting an array of master channel 
     *      roots of the contacts assigned to "trust" or "distrust" compiled from the MAM channel.
     */
    get: async (contactsRoot, iota) => {
...
const ratingFromMasterChannelRoot = async (sourceMasterChannelRoot, targetMasterChannelRoot, iota, depth = 5) => {
    const items = {}
...
tic.rating = {
     ...
     * @return {Promise.<RatingSearchResult>} a Promise awaiting the RatingSearchResult of the target.
     */
    fromMasterChannelRoot: async (sourceMasterChannelRoot, targetMasterChannelRoot, iota, depth = 5) => {
